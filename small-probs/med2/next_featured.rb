...
# Write a method that takes a single integer as an
# argument, and returns the next featured number that
# is greater than the argument. Issue an error message
# if there is no next featured number.

# Featured number stipulations:
# odd
# multiple of 7
# no repeating digits

# raise error if there is no next featured number
...
def featured(num)
  ...
  loop do
    break if uniq_digits?(num)
    raise error if num.digits.size == 11
    num += 14
  end
